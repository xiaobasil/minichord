...
bool MessageProcessor::handleMessage(std::string& message) {
	...
	str >> command;
#ifdef DEBUG
	cout<<"handleMessage command: "<<str.str()<<endl;
#endif
	...
	else if(command.compare("NOTIFY") == 0)
	{
		processNotify(str);
...
void MessageProcessor::processNotify(std::stringstream& stream)
{
	auto node = Node::createFromInfo(stream);
	if(chord->Predecessors[0]->getKey()==chord->NodeInfo->getKey()||Chord::inRange(chord->Predecessors[0]->getKey(),chord->NodeInfo->getKey(),node->getKey(),false,false))
	{
		chord->Predecessors[0]=node;
	}
}
